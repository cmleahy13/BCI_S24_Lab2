...
3. Why does the voltage on some of the channels have a positive peak are half a second after a target flash?
    
    At the foundational level, this experiment is using a P300 Speller, indicating that there is likely a "positive-going" peak around 300ms; however, the 300ms is based on the first P300 rather than what is typical, which would really be P500, suggesting the peak occurs 500ms (or 0.5s) after the event (although several of the subjects exhibit peaks roughly a quarter of a second after the event, which is consistent with the 300ms). Some reasons we could expect this behavior is that these channels may be more involved in the processing of the information presented (target or nontarget); peaks are pretty strongly limited to targets, as nontarget EEG data appears to follow a consistent pattern through the event occurrence. 
